...
test("Todoist update uses remoteUpdatedAt watermark instead of local updatedAt", async (t) => {
  const deps = makeDeps();
  const project = { id: "project-local", name: "Inbox", archived: false, externalLinks: [{ system: "todoist", projectId: "project-remote", remoteUpdatedAt: "2026-05-09T00:00:00.000Z" }], updatedAt: "2026-05-01T00:00:00.000Z" };
  const task = {
    id: "task-local",
    projectId: project.id,
    description: "Reschedule me",
    details: null,
    dueAt: "2026-05-10T00:00:00.000Z",
    status: "active",
    externalLinks: [{ system: "todoist", taskId: "todoist-task", remoteUpdatedAt: "2026-05-09T12:00:00.000Z" }],
    updatedAt: "2026-05-01T00:00:00.000Z",
  };
  const store = { projects: [project], tasks: [task] };
  const idx = makeIdx(store, deps);
  ...
  globalThis.fetch = async (url, init = {}) => {
    const parsed = new URL(url);
    requests.push({ method: init.method ?? "GET", pathname: parsed.pathname, body: init.body ? JSON.parse(init.body) : undefined });
    if (parsed.pathname === "/api/v1/tasks/todoist-task" && (init.method ?? "GET") === "GET") {
      return jsonResponse({ id: "todoist-task", content: "Reschedule me", description: "", project_id: "project-remote", due: { date: "2026-05-10" }, updated_at: "2026-05-09T12:00:00.000Z" });
    }
    if (parsed.pathname === "/api/v1/tasks/todoist-task" && init.method === "POST") {
      return jsonResponse({});
    }
    throw new Error(`Unexpected request: ${url}`);
  };
  ...
  await createTodoistProvider({ apiKey: "token" }, deps).updateTask({ store, idx, task, patch });

  assert.deepEqual(requests.map((request) => [request.method, request.pathname, request.body]), [
    ["GET", "/api/v1/tasks/todoist-task", undefined],
    ["POST", "/api/v1/tasks/todoist-task", { due_datetime: "2026-06-15T00:00:00.000Z" }],
    ["GET", "/api/v1/tasks/todoist-task", undefined],
  ]);
  assert.equal(patch.externalLinks[0].remoteUpdatedAt, "2026-05-09T12:00:00.000Z");
});
