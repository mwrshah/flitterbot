...
Determine which messages are *directly* relevant to the workstream being created. A message is relevant if it seems to be a continuation of the same task which led to the creation of the workstream but was being  discussed from before the exact current message that started / opened the workstream came through. Be conservative in marking messages as relevant. Do so when you are very sure and especially map if current message is a bit ambiguous and prior messages need to be considered to show the  full picture. 
...
3. The message tagged "CURRENT" (which triggered workstream creation) MUST be marked true.
