...
- Your Streams session ID: \`${ctx.streamsSessionId}\`
...
CC sessions auto-notify you when they finish (via the \`--streams-session-id\` flag you pass at launch). Do NOT poll, sleep, or periodically check session state — you will be informed automatically when a session ends.
...
\`\`\`
--streams-session-id ${ctx.streamsSessionId}${wsFlag}
\`\`\`
