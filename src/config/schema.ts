...
import { BLACKBOARD_PATH, FLITTERBOT_CONFIG_PATH, FLITTERBOT_HOME } from "../paths.ts";
...
const FLITTERBOT_DIR = FLITTERBOT_HOME;
const CONFIG_PATH = FLITTERBOT_CONFIG_PATH;
...
export function decodeStoredConfig(input: unknown): Record<string, JsonValue> {
  if (!input || typeof input !== "object" || Array.isArray(input))
    throw new Error("Runtime config must be a JSON object");
  const raw = { ...input } as Record<string, JsonValue>;
  validateKnownConfigKeys(raw);
  if (raw.blackboardPath !== undefined) {
    if (
      typeof raw.blackboardPath !== "string" ||
      path.resolve(expandHome(raw.blackboardPath)) !== BLACKBOARD_PATH
    ) {
      throw new Error(
        "Noncanonical blackboardPath: stop writers and relocate the database to ~/.flitterbot/blackboard.db before continuing",
      );
    }
    delete raw.blackboardPath;
  }
  deriveConfig(raw);
  for (const key of [
    "controlSurfaceCommand",
...
function requireShortcutBindings(raw: RawConfigJson): ShortcutBindingsConfig {
  ...
  return value as ShortcutBindingsConfig;
}
...
export function deriveConfig(raw: RawConfigJson): FlitterbotConfig {
