...
- **New swimlane needed** → default agent (can call `create_swimlane` to spawn orchestrator)
...
The default agent creates at most one swimlane per user turn via `create_swimlane`. The tool inserts a SQLite row, spawns a bound orchestrator, and always passes relevant user context through to the new stream. Stream insertion and rename share one case-insensitive collision contract: the first requested `name` stays `name`, then collisions become `2name`, `3name`, and so on. Forking uses that same insertion contract, while persisted historical names remain unchanged. For direct creation, the runtime looks at up to 10 recent default-surface real user messages (`web`/`whatsapp`, `sender=user`, no `stream_id`) after the previous stream creation boundary, asks a Groq relevance classifier which messages belong in the new stream, forces the current user message in if missing, and formats those messages as the orchestrator's initial prompt. The relevance classifier sees the stream name, the default agent's optional `message` as the stream purpose/agent context, and the candidate user messages; it is instructed to omit vague default-agent orchestration prompts unless that purpose makes the concrete task clear. If relevance classification fails, it falls back to the current user message only.

When the user explicitly asks to prepare staged or multi-task handoffs, the default agent calls `prep_launch` once with a `launches` array instead of creating streams. Each array item is canonical `SwimlaneLaunchArgs` (`suggestedName`, `cwd`, `initialMessage`, optional `additionalContext`) and becomes one card through the existing Pi tool timeline. The web chat renders each item as editable JSON with its own launch arrow. Each rendered card keeps edits and launch status in local component state; remounting the row or refreshing restores the tool-produced JSON. Each arrow separately posts its card's current JSON to `POST /api/pi-sessions/:id/prepared-swimlanes`, which derives ownership from the source Pi session, uses the existing stream spawn primitive, and enqueues the targeted initial orchestrator prompt. The scratch-swimlane `POST /api/streams` contract remains `{ name?, cwd? }`. No edit persistence, replay state, or JSONL mutation exists.
...
**Default** — real always-on triage session. Created first at startup via `PiSessionManager.createDefault()` → `createFlitterbotAgent({ role: "default" })` → `buildDefaultAgentPrompt(...)`. Delegates one immediate unit of engineering work via `create_swimlane`, which always passes relevant user context. Uses one batch-shaped `prep_launch` call to emit editable launch cards when the user wants staged or multiple handoffs. Sends messages to orchestrators via `enqueue_message`. Cannot write code.
...
| 22 | [Prepared Swimlane Launch](prepared-swimlane-launch/FEATURE.md) | Editable in-chat launch arguments for reviewed, user-triggered swimlane creation without replay or edit persistence |
...
`open` (active, optional repo/worktree) · `closed` (soft-deleted with `closed_at`; visible for 7d, or indefinitely while pinned, for status reporting and reopening).
