...
When the active route changes, `setupWsRouteSubscriptions()` clears the previous session's transient state before it changes the server subscription. Before position acceptance or dispatch, `FlitterbotWsClient` rejects session events that do not match the active `piSessionId`, or the active wildcard subscription's event-type filter. `stream_surfaced` always reaches the shared surface cache. Scoped errors also pass: active-session errors recover immediately, while inactive-session errors mark that history stale without reloading it or showing a toast. `<StreamsMessageList key={piSessionId}>` remounts the virtualizer and initial-bottom state for the new conversation.
